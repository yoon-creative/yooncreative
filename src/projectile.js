...
export class Projectile {
  constructor(tower, x, y, tx, ty, target) {
    this.tower = tower; this.typeId = tower.typeId;
    this.x = x; this.y = y; this.target = target; this.dead = false;
    const s = tower.stats();
    this.dmg = s.dmg; this.splash = s.splash || 0; this.slow = s.slow || 0;
    this.slowDur = s.slowDur || 0; this.stunDur = s.stunDur || 0;
    this.chain = s.chain || 0; this.chainRange = (s.chainRange || 0) * GRID.tileSize;
    this.color = s.projColor; this.speed = s.projSpeed * GRID.tileSize;
    const dx = tx - x, dy = ty - y, d = Math.hypot(dx, dy) || 1;
    this.vx = dx / d * this.speed; this.vy = dy / d * this.speed;
    this.life = 2.5; this.trail = [];
  }
  ...
  update(dt, enemies, effects) {
    ...
    if (!hit) return;
    this._applyHit(hit, enemies, effects); this.dead = true;
  }
  ...
  _applyHit(target, enemies, effects) {
    ...
    if (this.splash > 0) {
      const r = this.splash * GRID.tileSize, r2 = r * r;
      for (const e of enemies) {
    ...
    if (this.chain > 0) {
      let prev = target; const hitSet = new Set([target]);
      for (let i = 0; i < this.chain; i++) {
        let next = null, bestD = this.chainRange * this.chainRange;
        for (const e of enemies) {
