...
export class Grid {
  ...
  getTower(c, r) { if (!this.inBounds(c, r)) return null; return this.cells[this.idx(c, r)]; }
  isBlocked(c, r) { if (!this.inBounds(c, r)) return true; return this.cells[this.idx(c, r)] !== null; }
  isReserved(c, r) { return (c === this.spawn.col && r === this.spawn.row) || (c === this.exit.col && r === this.exit.row); }
  ...
  worldToCell(wx, wy) { return { col: Math.floor(wx / this.tile), row: Math.floor(wy / this.tile) }; }
  cellCenterWorld(c, r) { return { x: c * this.tile + this.tile / 2, y: r * this.tile + this.tile / 2 }; }
  forEachTower(fn) { for (let i = 0; i < this.cells.length; i++) { const t = this.cells[i]; if (t) fn(t); } }
}